...
1. **動作確認**
   ...
   - 総ドキュメント数（100）と最終更新日時が表示されることを確認
...
3. **将来の実装**
   - データベース接続後、`backend/src/services/statsService.ts` のモックデータを実際の統計情報取得に置き換える
...
1. **データベース統合**
   - PostgreSQLからの実データ取得
   - `statsService.ts` のモックデータを実装に置き換え
