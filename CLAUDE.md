...
- 詳細は `.claude/rules/troubleshooting.md` を参照。
- エラー解決で得た知見を上記に蓄積すること。
...
- モード未指定時は `hybrid` をデフォルト適用、不正値には 400 Bad Request を返す
- 全モードでスコア情報を含む統一レスポンス形式（`{ results: [{ ..., score }] }`）を実装
- テスト78件すべて成功（74件 → 78件）、カバレッジ84.77%達成（search.ts: 97.61%）
- コード品質評価：保守性・安全性・パフォーマンスすべて優秀
